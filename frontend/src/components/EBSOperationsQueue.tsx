  ...
  return <section className="panel operations-queue" id="operations-queue">
    ...
    {error ? <div className="registry-empty"><strong>Protected operations queue</strong><p>{error === 'Authentication required' ? 'Sign in with your DHIS2 account to manage assignments and deadlines.' : error}</p></div> : !data ? <p className="registry-message">Loading operational signals…</p> : <>
      <div className="queue-summary">
        {[['OPEN', data.summary.open], ['HIGH RISK', data.summary.high_risk], ['OVERDUE', data.summary.overdue], ['DUE SOON', data.summary.due_soon], ['UNASSIGNED', data.summary.unassigned], ['CLOSED', data.summary.closed]].map(([name, count]) => <button key={name} className={Number(count) > 0 && ['OVERDUE', 'UNASSIGNED'].includes(String(name)) ? 'has-alert' : ''} type="button" onClick={() => setFilter(String(name).replace(' ', '_'))}><span>{name}</span><strong>{count}</strong></button>)}
      </div>
      <div className="operations-workspace">
        <div className="queue-main">
          ...
          <div className="table-scroll"><table><thead><tr><th>Signal</th><th>Stage</th><th>Risk</th><th>Assigned officer</th><th>Deadline</th><th>Status</th><th /></tr></thead><tbody>{visible.map((item) => <tr key={item.tracked_entity_uid} className={item.overdue ? 'overdue-row' : ''}><td><strong>{item.signal_id}</strong><small>{item.title}</small></td><td>{label(item.latest_stage)}</td><td><RiskBadge level={badgeRisk(item.risk_level)} /></td><td>{item.responsible_officer ?? <span className="unassigned-text">Unassigned</span>}</td><td><DueStatus item={item} /></td><td><span className={`queue-status ${item.closed ? 'closed' : ''}`}>{item.closed ? 'Closed' : item.response_status ? label(item.response_status) : 'Open'}</span></td><td><button className="row-action" type="button" disabled={item.closed} onClick={() => openAssignment(item)}>{item.responsible_officer ? 'Update' : 'Assign'}</button></td></tr>)}</tbody></table>{visible.length === 0 && <p className="empty-filter">No alerts match this view.</p>}</div>
        </div>
