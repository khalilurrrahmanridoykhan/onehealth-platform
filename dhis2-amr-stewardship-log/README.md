...
4. **Submitting an entry, or a follow-up/approval update, is not gated by this app's UI at all.** DHIS2's own org-unit capture-scope check on `POST /api/tracker` is the real, server-enforced boundary -- confirmed live (see above) to return a genuine `409` for an out-of-scope account, not a silent failure.
