...
export function RiskMap({ items, selected, diseaseName, metricLabel, onSelect }: Props) {
  ...
  useEffect(() => {
    setLayer(diseaseName.includes('HPAI') || diseaseName.includes('Nipah') || diseaseName.includes('Japanese') || diseaseName.includes('Watery') ? 'total' : diseaseName === 'Measles' ? 'cases' : 'risk')
  }, [diseaseName])
