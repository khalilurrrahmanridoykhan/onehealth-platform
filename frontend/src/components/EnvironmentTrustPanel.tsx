...
import {
  fetchEnvironmentTrustReport,
  type EnvironmentTrustCapabilities,
  type EnvironmentTrustReport,
} from '../environmentTrust'

interface EnvironmentTrustPanelProps {
  report?: EnvironmentTrustReport
}
...
function CapabilityList({ capabilities }: { capabilities: EnvironmentTrustCapabilities }) {
  const rows: Array<[string, boolean]> = [
    ...
    ['Disease correlation analysis', capabilities.disease_correlation],
  ]
...
function EnvironmentTrustContent({ report }: { report: EnvironmentTrustReport }) {
  ...
  return <section className="panel data-trust-panel" aria-labelledby={headingId} id="environment-trust">
    <div className="panel-heading data-trust-heading">
      <div>
        <p className="eyebrow">Data trust &amp; operations</p>
        <h2 id={headingId}>Environment evidence, freshness and provenance</h2>
        <p className="data-trust-intro">What this climate dataset can support—and where it must not be over-interpreted.</p>
      </div>
    ...
    <div className="data-trust-grid">
      <article className="data-trust-card">
        ...
        <dl className="data-trust-details">
          ...
          <div><dt>Districts</dt><dd>{number.format(report.coverage.location_count)}</dd></div>
    ...
    <div className="data-trust-assurance-grid">
      ...
      <aside className="data-trust-limitations" aria-labelledby={`${headingId}-limitations`}>
        ...
        {report.limitations.length ? <ul>{report.limitations.map((limitation) => <li key={limitation}>{limitation}</li>)}</ul>
          : <p>No additional limitations were documented.</p>}
      </aside>
    </div>
  </section>
}

export function EnvironmentTrustPanel({ report: suppliedReport }: EnvironmentTrustPanelProps) {
  const [report, setReport] = useState<EnvironmentTrustReport | undefined>(suppliedReport)
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError('')
    fetchEnvironmentTrustReport(controller.signal)
      .then(setReport)
      .catch((reason: unknown) => {
        if (!controller.signal.aborted) setError(reason instanceof Error ? reason.message : 'Could not load environment data-trust metadata.')
      })
      .finally(() => {
        if (!controller.signal.aborted) setLoading(false)
      })
    return () => controller.abort()
  }, [attempt, suppliedReport])

  if (loading) return <section className="panel data-trust-panel data-trust-loading" aria-busy="true" aria-live="polite"><p>Loading environment data-trust evidence…</p></section>
  if (error) return <section className="panel data-trust-panel data-trust-error" role="alert">
    <h2>Environment data-trust evidence is unavailable</h2>
    <p>{error}</p>
    <button type="button" onClick={() => setAttempt((value) => value + 1)}>Try again</button>
  </section>
  return report ? <EnvironmentTrustContent report={report} /> : null
}
