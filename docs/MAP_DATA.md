...
The raw ADM2 source also has counterclockwise exterior polygon rings, the opposite of the convention this project's division map already uses. Rendered as-is, `geoPath`/`geoMercator` interprets each district as its world-covering inverse — a single filled rectangle instead of a district shape (this is the same failure mode the polygon-ring note above warns about for the division map). `scripts/build_environment_map.py` rewinds every ring to clockwise before writing the output file. If you ever regenerate this file from a different source, re-check winding the same way, or the map will silently render as one solid rectangle again.
