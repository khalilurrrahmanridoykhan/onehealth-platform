...
This document describes how data moves through the OneHealth platform, what the current milestone actually implements, and where the boundary sits between this application and DHIS2. It is the technical companion to the [Data Trust and Operations guide](DATA_TRUST.md) and the [ingestion approval guide](INGESTION_GATE.md).

## Contents

- [Current data path](#current-data-path)
- [Current milestone](#current-milestone)
- [Data Trust and Operations layer](#data-trust-and-operations-layer)
- [Implementation status](#implementation-status)
- [DHIS2 system boundary](#dhis2-system-boundary)
...
```mermaid
flowchart TD
    A["<b>Public aggregate and research sources</b>"] --> B["<b>Source-specific import adapters</b>"]
    B --> C["<b>Normalized aggregate CSV records</b>"]

    C -->|CSV read backend| G["<b>Process-local snapshot cache</b><br/>TTL + stale-on-error state"]

    C --> D["<b>Quality report + staged review package</b>"]
    D --> E["<b>Explicit data-steward approval</b>"]
    E --> F["<b>Approved DHIS2 dry-run / explicit commit</b>"]
    F --> H["<b>DHIS2 aggregate data backend</b>"]
    H -->|DHIS2 read backend| G

    G --> I["<b>Data Trust derivation</b><br/>registry + runtime records"]
    G --> J["<b>Trends, alerts, summaries</b>"]
    I --> K["<b>Public FastAPI reads</b>"]
    J --> K
    K --> L["<b>Customized React dashboard</b><br/>Data Trust · maps · alerts · operations"]
```
...
All surveillance reads share a process-local snapshot keyed by the configured CSV paths or DHIS2 connection/mapping/date-range settings.

| Setting | Controls | Default |
|---|---|---|
| `ONEHEALTH_CACHE_TTL_SECONDS` | Fresh lifetime of a snapshot | 300 seconds |
| `ONEHEALTH_CACHE_STALE_IF_ERROR_SECONDS` | Maximum age served after a refresh error | 86,400 seconds |

Values must be non-negative numbers; the stale window is never shorter than the TTL.
...
Quality status is derived from checks for records, disease identity, period ranges, non-negative values, duplicates, record-level source provenance, allowed evidence semantics, partial periods, declared location coverage, and future periods. Any failed check produces `FAIL`; otherwise warnings produce `WARNING`, and a clean report produces `PASS`.

> [!NOTE]
> These reports describe the evidence available to the application; they are not clinical validation or official public-health approval.
...
DHIS2 is responsible in the target deployment for organisation units, aggregate surveillance values, EBS signals and verification state, investigations, response actions where appropriate, user permissions, and auditable programme history.

The OneHealth application is responsible for source-specific normalization, evidence and quality reporting, guarded synchronization, cached API delivery, prediction and explainable alerts, and the customized dashboard experience.
