...
export function SessionControl() {
  ...
  return <div className="session-control"><button type="button" onClick={() => setOpen(!open)}>Sign in</button>{open && <form className="login-popover" onSubmit={submit}><strong>Protected EBS access</strong><label>Username<input required value={username} onChange={(event) => setUsername(event.target.value)} /></label><label>Password<input required type="password" value={password} onChange={(event) => setPassword(event.target.value)} /></label>{error && <p role="alert">{error}</p>}<button type="submit">Continue</button></form>}</div>
}
