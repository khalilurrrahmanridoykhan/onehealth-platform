...
export function TrendChart({ records, alert, diseaseName, metricLabel, periodLabel }: { records: SurveillanceRecord[]; alert?: Alert; diseaseName: string; metricLabel: string; periodLabel: string }) {
    ...
    <section className="panel chart-panel">
      <div className="panel-heading">
        <div>
          ...
          <h2>Observed trend and next-{periodLabel} outlook</h2>
        </div>
        <span className="source-label">{diseaseName.includes('HPAI') ? 'WOAH WAHIS' : 'DGHS aggregate surveillance'}</span>
      </div>
