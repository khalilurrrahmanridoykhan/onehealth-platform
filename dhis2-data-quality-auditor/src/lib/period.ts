import type { PeriodType } from '../types/audit'
...
// Last calendar day of `month` (0-indexed) in `year`, via day-0-of-next-month.
...
export function parsePeriod(periodType: PeriodType, id: string): ParsedPeriod {
  if (periodType === 'Daily') {
    ...
    if (!match) throw new Error(`Invalid daily DHIS2 period: ${id}`)
    const [, y, m, d] = match
  ...
  if (periodType === 'Monthly') {
    ...
    if (!match) throw new Error(`Invalid monthly DHIS2 period: ${id}`)
    const year = Number(match[1])
    const month = Number(match[2]) - 1
    if (month < 0 || month > 11) throw new Error(`Invalid monthly DHIS2 period: ${id}`)
    return { id, start: new Date(Date.UTC(year, month, 1)), end: lastDayOfMonth(year, month) }
  }

  if (periodType === 'Quarterly') {
    const match = /^(\d{4})Q([1-4])$/.exec(id)
    if (!match) throw new Error(`Invalid quarterly DHIS2 period: ${id}`)
    const year = Number(match[1])
    const startMonth = (Number(match[2]) - 1) * 3
    return { id, start: new Date(Date.UTC(year, startMonth, 1)), end: lastDayOfMonth(year, startMonth + 2) }
  }
