  ...
  return (
    <section className="summary-grid" aria-label="Surveillance summary">
      ...
      <article className="metric-card">
        <span>{isNipah ? 'Latest reported deaths' : isRabies ? 'Geographic resolution' : `Expected ${metricLabel}`}</span>
        <strong>{isNipah ? number.format(summary?.latest_deaths ?? 0) : isRabies ? 'National' : alert ? number.format(Math.round(alert.expected_cases)) : '—'}</strong>
        <small>{isNipah ? summary?.latest_period : isRabies ? 'WHO GHO NTD_RAB2' : 'Previous four-period mean'}</small>
      </article>
      <article className="metric-card">
        <span>{isNipah ? 'Cumulative case fatality' : isRabies ? 'Observed annual series' : 'Change from baseline'}</span>
        <strong className={change !== null && change > 0 ? 'text-alert' : 'text-stable'}>
          {isNipah ? cfr === null ? '—' : `${cfr.toFixed(1)}%` : isRabies ? `${summary?.periods ?? 0} years` : change === null ? '—' : `${change > 0 ? '+' : ''}${change.toFixed(1)}%`}
        </strong>
        <small>{isNipah ? `${number.format(summary?.total_deaths ?? 0)} deaths` : isRabies ? '2015–2024 historical reporting' : `Latest complete ${periodLabel}`}</small>
      </article>
      <article className="metric-card">
