...
**The actual fix: grant DHIS2-native data-viewing apps instead of trying to open this one.** The shared "Data Share Hub - Read Only" role's `authorities` (see `lib/serviceAccount.ts`'s `buildUserRolePayload`) are `M_dhis-web-dashboard` and `M_dhis-web-data-visualizer` -- confirmed live that each individually unlocks only that one app, nothing broader. The recipient can browse their shared data in DHIS2's own polished Dashboard/Data Visualizer apps after logging in, which is arguably a better outcome than a hand-built table anyway. `RecipientView` still exists and still works for any account that *can* reach it (e.g. if a future DHIS2 version changes this behavior), but nothing in this app depends on the recipient reaching it.

**A related self-healing fix worth knowing about:** `hooks/useCreateServiceAccount.ts`'s `findOrCreateRole` doesn't just create the shared role once and cache its id forever -- every share creation re-checks the existing role's authorities against `buildUserRolePayload()` and updates them if they've drifted. This matters because the role's required authorities changed after this app had already been used to create real shares (from empty to Dashboard+Data Visualizer) -- without this sync, any role created before that change would have silently kept its stale, more-restrictive authorities.
